...
<script lang="ts">
	...
	const assignedDMIds = $derived(data.region.dms.map((d: any) => d.dmProfileId));
	const availableDMs  = $derived(data.allDMs.filter((d: any) => !assignedDMIds.includes(d.id)));
...
<div class="page">
	<div class="page__header">
		<div>
			<a href="/world/{data.region.worldId}" class="back-link">← {data.region.world.name}</a>
	...
	{#if (form as any)?.dmSuccess}<div class="form-success">DM assignment updated.</div>{/if}
	...
	<div class="sections">
		...
		<div class="card">
			...
			{:else}
				<p class="table__empty" style="margin-bottom:0.75rem;">No DMs assigned.</p>
			{/each}
					...
					<div style="display:flex; gap:0.5rem;">
						<select name="dmProfileId" class="input" style="flex:1;">
							...
							{#each availableDMs as dm}
								<option value={dm.id}>{dm.user?.name ?? dm.userId}</option>
							{/each}
						</select>
					...
					{#each data.region.locations as loc}
						<tr>
							...
							<td class="table__action">
								<a href="/world/{data.region.worldId}/regions/{data.region.id}/locations/{loc.id}" class="btn btn-ghost btn-sm">Edit</a>
							</td>
						</tr>
					{/each}
				</tbody>
