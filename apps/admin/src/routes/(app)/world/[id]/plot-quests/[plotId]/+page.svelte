<!-- apps/admin/src/routes/(app)/world/[id]/plot-quests/[plotId]/+page.svelte -->
<script lang="ts">
	import { enhance } from '$app/forms';
	import { invalidateAll } from '$app/navigation';
	import { ConfirmModal, FantasyDateField } from '@core/ui';
...
<div class="page__header">
	<div>
		<a href="/world/{data.world.id}/plot-quests" class="back-link">← Plot quests</a>
...
<div class="sections" style="display:grid; gap:1rem;">
	<div class="card">
	...
	<div class="card">
		...
		{#each plot.factions as link (link.id)}
			<div class="faction-subrow">
				<span class="faction-subrow__grow" style="font-weight:600;">
					{#if link.faction}
						<a href="/world/{data.world.id}/factions/{link.faction.id}">{link.faction.name}</a>
					{:else}
						(missing faction)
					{/if}
				</span>
	...
	<div class="card">
		...
		{#each plot.npcs as link (link.id)}
			<div class="faction-subrow">
				<span class="faction-subrow__grow" style="font-weight:600;">
					{#if link.npc}
						<a href="/world/{data.world.id}/npcs/{link.npc.id}">{link.npc.name}</a>
					{:else}
						(missing NPC)
					{/if}
				</span>
