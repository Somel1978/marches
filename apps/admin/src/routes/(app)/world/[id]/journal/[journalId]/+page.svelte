...
<div class="page">
	<div class="page__header">
		<div>
			<a href="/world/{worldId}/journal" class="back-link">← World Journal</a>
					...
					<div style="margin-bottom:0.75rem;padding:0.5rem;background:var(--bg-overlay);border-radius:var(--radius-md);">
						...
						{#each section.pages ?? [] as pg}
							<button type="button"
								...
								onclick={() => goto(`/world/${worldId}/journal/${journal.id}?page=${pg.id}`)}>
								{pg.title}
							</button>
						{/each}
...
{#if activePage}
	<form id="del-pg-form" method="post" action="?/deletePage" use:enhance={() => {
		return async ({ update }) => { goto(`/world/${worldId}/journal/${journal.id}`); await update(); };
	}} style="display:none;">
		<input type="hidden" name="id" value={activePage.id} />
	</form>
{/if}
