...
function traitGrantFields(data: FormData) {
	return {
		...
		resistanceChoiceCount:     data.get('resistanceChoiceCount') ? Number(data.get('resistanceChoiceCount')) : null,
		resistanceChoicePool:      data.get('resistanceChoicePool')?.toString().trim()      || null,
		immunityChoiceCount:       data.get('immunityChoiceCount') ? Number(data.get('immunityChoiceCount')) : null,
		immunityChoicePool:        data.get('immunityChoicePool')?.toString().trim()        || null,
		vulnerabilityChoiceCount:  data.get('vulnerabilityChoiceCount') ? Number(data.get('vulnerabilityChoiceCount')) : null,
		vulnerabilityChoicePool:   data.get('vulnerabilityChoicePool')?.toString().trim()   || null,
		grantsFeatCategory:    data.get('grantsFeatCategory')?.toString().trim()    || null,
		grantsFeatId:          data.get('grantsFeatId')?.toString().trim()          || null,
