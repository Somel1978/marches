...
- `components.css` — Tailwind `@layer components`: all reusable visual patterns. No `<style>` blocks in pages — pages use class names only.

**Rule: no `<style>` blocks in pages.** All visual patterns live in `components.css`. If a component needs slots or logic it becomes a Svelte component in `@core/ui/components/`. Pages only use class names.

**Key component classes:** `.page`, `.page__header`, `.sections` (responsive 2-col grid), `.card`, `.btn`, `.badge`, `.input`, `.field`, `.form-error`, `.form-success`, `.form-actions`, `.table`, `.table-wrap` (scroll wrapper), `.col-hide-mobile`, `.col-hide-tablet`, `.pagination`, `.auth-card`, `.role-list`, `.role-option`, `.matrix`, `.detail-panel`, `.site`, `.nav-bar`, `.landing`, `.dashboard`
...
src/routes/
  ...
  (protected)/
    ...
    profile/                  View/edit own profile (name, image, discord, mobile)
