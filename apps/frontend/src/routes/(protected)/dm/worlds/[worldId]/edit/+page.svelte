		...
		<form method="post" action="?/updateWorld" use:enhance={() => {
			...
			<div class="fields">
				<div class="field">
					...
					<input id="wname" name="name" type="text" class="input" value={world.name} required />
				</div>
				<div class="field">
					<label class="label" for="wdesc">Description</label>
					<textarea id="wdesc" name="description" class="input" rows="2">{world.description ?? ''}</textarea>
				</div>
				<div class="field">
					<label class="label" for="wmap">Map image URL</label>
					<input id="wmap" name="mapImageUrl" type="url" class="input" value={world.mapImageUrl ?? ''} placeholder="https://..." />
				</div>
				<div class="field field--inline">
					<label class="label" for="wactive">Active</label>
					<select id="wactive" name="isActive" class="input input--select">
						<option value="true"  selected={world.isActive}>Yes</option>
						<option value="false" selected={!world.isActive}>No</option>
					</select>
				</div>
				<div class="field">
					...
					<select id="w-global" name="acceptsGlobalCharacters" class="input input--select">
						<option value="true"  selected={(world as any).acceptsGlobalCharacters !== false}>Accept global characters</option>
						<option value="false" selected={(world as any).acceptsGlobalCharacters === false}>World-specific characters only</option>
					</select>
		...
		{#if world.mapImageUrl}
			{#if placingMarker}
				<button type="button"
					...
					onclick={onMapClick}>
					<img src={world.mapImageUrl} alt="World map" style="width:100%; display:block; border-radius:var(--radius-md);" />
...
<div class="card">
	<div class="page__header" style="margin-bottom:1rem;">
		<h3 class="section-title" style="margin:0;">Regions ({world.regions.length})</h3>
			...
			<tbody>
				{#each world.regions as region}
					<tr>
						...
						<td class="table__muted col-hide-mobile">{region.locations?.length ?? 0}</td>
						<td>
							{#if world.mapImageUrl}
						...
						<td class="table__action">
							<a href="/dm/worlds/{world.id}/regions/{region.id}" class="btn btn-ghost btn-sm">Edit</a>
						</td>
					</tr>
				{:else}
