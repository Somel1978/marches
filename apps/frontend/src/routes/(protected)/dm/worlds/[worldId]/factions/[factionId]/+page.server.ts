...
export const actions: Actions = {
	...
	delete: async ({ params, locals }) => {
		...
		throw redirect(303, `/dm/worlds/${params.worldId}/factions`);
	},
	...
	updateRank: async ({ params, request, locals }) => {
		if (!await assertCanManage(params.worldId, locals.user!.id)) return fail(403, { message: 'Forbidden' });
		const data   = await request.formData();
	...
	deleteRank: async ({ params, request, locals }) => {
		if (!await assertCanManage(params.worldId, locals.user!.id)) return fail(403, { message: 'Forbidden' });
		const data   = await request.formData();
		const rankId = data.get('rankId')?.toString() ?? '';
		if (!rankId) return fail(400, { message: 'Rank required.' });
	...
	removeTerritory: async ({ params, request, locals }) => {
		if (!await assertCanManage(params.worldId, locals.user!.id)) return fail(403, { message: 'Forbidden' });
		const data        = await request.formData();
	...
	removeRelation: async ({ params, request, locals }) => {
		if (!await assertCanManage(params.worldId, locals.user!.id)) return fail(403, { message: 'Forbidden' });
		const data       = await request.formData();
