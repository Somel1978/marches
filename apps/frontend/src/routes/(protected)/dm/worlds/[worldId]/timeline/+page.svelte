...
<script lang="ts">
	...
	function hrefFor(entry: TimelineEntryView) {
		if (entry.kind === 'PLOT_QUEST') return `/dm/worlds/${data.world.id}/plot-quests/${entry.hrefKey}`;
		if (entry.kind === 'NPC_SCHEDULE' && entry.npcId) return `/dm/worlds/${data.world.id}/npcs/${entry.npcId}`;
		if (entry.kind === 'WEATHER' && entry.regionId) return `/dm/worlds/${data.world.id}/regions/${entry.regionId}`;
		return null;
	}
...
<div class="page__header" style="margin-bottom:0.75rem;">
	<div>
		...
		<p style="margin:0.25rem 0 0; color:var(--text-muted); font-size:0.875rem;">
			...
			{#if data.canManage}
				<a href="/dm/worlds/{data.world.id}/calendar">Edit calendar</a>
