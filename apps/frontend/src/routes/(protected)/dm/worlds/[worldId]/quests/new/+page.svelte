<!-- apps/frontend/src/routes/(protected)/dm/worlds/[worldId]/quests/new/+page.svelte -->
<script lang="ts">
	import { enhance } from '$app/forms';
	import type { PageData, ActionData } from './$types';
	import EncounterPlannerPanel from '../../../../quests/_planner/EncounterPlannerPanel.svelte';
	import { defaultPlannerState } from '../../../../quests/_planner/planner.ts';
	...
	// allWorlds = DM's assigned worlds (from layout myWorlds), full regions + locations included
	const _allWorlds = $derived(((data as any).allWorlds ?? []) as any[]);

	// Pre-select the current world from the URL context; DM can switch if assigned to others
	let selectedWorldId    = $state('');
	let selectedRegionId   = $state('');
	let selectedLocationId = $state('');

	// Set initial world once data is available
	$effect.pre(() => {
		const worldId = ((data as any).world as any)?.id ?? '';
		if (worldId && !selectedWorldId) selectedWorldId = worldId;
	});
...
<div class="card" style="max-width:{tab === 'planner' ? 'none' : '600px'};">
	...
	<form method="post" use:enhance={() => {
		...
		<!-- World / Region / Location — world pre-selected, still switchable -->
		<div class="field">
			...
			<select id="q-world" class="input input--select"
				...
				<option value="">No world (global quest)</option>
		...
		<div class="form-actions">
			<a href="/dm/worlds/{(data as any).world?.id}/quests" class="btn btn-ghost">Cancel</a>
