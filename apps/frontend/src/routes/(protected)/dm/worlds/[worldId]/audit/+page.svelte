...
<div class="card" style="margin-bottom:1.5rem;">
	<div style="display:flex; gap:0.75rem; flex-wrap:wrap; align-items:flex-end;">
		...
		{#if filters.resourceKey || filters.action || filters.from || filters.to}
			<a href="?" class="btn btn-ghost btn-sm" style="align-self:flex-end;">Clear</a>
		{/if}
	</div>
</div>
		...
		<table class="table">
			...
			<tbody>
				{#each data.items as log}
					<tr>
						<td class="table__muted" style="font-size:0.8125rem;">{formatDate(log.createdAt)}</td>
						<td>
							{#if (log as any).actor}
								<span style="font-weight:500;">{(log as any).actor.name}</span>
								<span class="table__muted" style="display:block; font-size:0.75rem; overflow:hidden; text-overflow:ellipsis; max-width:120px;">{(log as any).actor.email}</span>
							{:else}
