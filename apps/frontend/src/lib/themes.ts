...
// Parses available user-selectable themes from the shared tokens.css file.
// Convention: any [data-theme="frontend-*"] block preceded by /* theme-name: X */ is selectable.
// The default [data-theme="frontend"] is always included as "Default".
// Uses Vite's ?raw import — works in both dev and production with no filesystem access.
...
export function getAvailableThemes(): ThemeOption[] {
    ...
    // Always include the default frontend theme first
    const defaultTokens = extractTokens(tokensCSS, 'frontend');
    themes.push({
        key:         'frontend',
        name:        'Default',
        bgBase:      defaultTokens['--bg-base']       ?? '#14171C',
        bgSurface:   defaultTokens['--bg-surface']    ?? '#1E2228',
    ...
    // Find all /* theme-name: X */ immediately followed by [data-theme="frontend-*"]
    // \s* only — no arbitrary CSS allowed between comment and selector (prevents
    // the Default comment from being matched to a distant frontend-* selector)
    const pattern = /\/\*\s*theme-name:\s*([^*]+)\*\/\s*\[data-theme="(frontend-[^"]+)"\]/g;
    let match;
    while ((match = pattern.exec(tokensCSS)) !== null) {
        ...
        themes.push({
            key,
            name,
            bgBase:      tokens['--bg-base']             ?? '#14171C',
            bgSurface:   tokens['--bg-surface']          ?? '#1E2228',
...
export function validateTheme(theme: string): string {
    ...
    return valid.includes(theme) ? theme : 'frontend';
}