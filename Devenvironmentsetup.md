...
> **Next session starting point.** Complete the dev environment setup first, then proceed to the Journal/Wiki refactor.
...
### Root Cause
ASI slots are computed by `getCharacterSheet` which runs post-creation. The wizard has no ASI step. The data needed is already available client-side (`systemData.classes` includes full features with `requiredLevel`).

### Solution — Add ASI Step to Wizard

**Step 1 — Compute ASI slots client-side in the wizard**

Same logic as `get-character-sheet.ts` lines 62-84:
- For each class allocation, filter features where `requiredLevel <= allocatedLevel`
- Features named `"Ability Score Improvement"` → ASI slot
- Features named `"Epic Boon Feat"` → Epic Boon slot
- Result: array of `{ classId, classLevel, type: 'asi' | 'epic_boon' }`

**Step 2 — Insert conditional ASI step between Classes and Review**

```
Identity → Species → Background → Scores → Classes → [ASI if slots exist] → Review
```

- Only show if `asiSlots.length > 0`
- STEPS array becomes dynamic based on computed slots
- Same UI as the existing `Dnd5eAsiFeatsPanel` component (already in `@core/ui`)

**Step 3 — Submit ASI choices with character creation**

In `+page.server.ts` create action, after `dnd5e.createCharacter()`:
- Call `dnd5e.saveAbilityScores()` (already done)
- Call `dnd5e.saveAsiChoices(character.id, asiChoices)` if any ASI choices made
- Call `dnd5e.addCharacterFeat(character.id, featId, { sourceClassId, sourceLevel })` for each feat pick

**Files to change:**
- `apps/frontend/src/routes/(protected)/characters/new/dnd5e/+page.svelte` — add ASI step, slot computation, choices state
- `apps/frontend/src/routes/(protected)/characters/new/dnd5e/+page.server.ts` — save ASI choices + feat picks post-creation

**Note:** Background feat pick is already handled in the wizard. This only adds class-level ASI/feat slots.
